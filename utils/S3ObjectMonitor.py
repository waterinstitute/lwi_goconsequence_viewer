...
class S3ObjectMonitor:
    ...
    def monitor_objects(self):
        ...
        for _b in self.buckets:
            try:
                shp_files.extend(
                        ...
                        {"Bucket":_b,"Key":x["Key"],"LastModified":x["LastModified"].isoformat()}
        ...
        updated_objects = [obj for obj in current_state if datetime.fromisoformat(obj["LastModified"])>datetime.fromisoformat(last_run)]
        ...
        # Last Run update
        self.save_last_run()
        return {"added":added_objects,"removed":removed_objects,"updated":updated_objects}